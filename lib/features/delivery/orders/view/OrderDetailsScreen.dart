...
//             delivery(localisation_delivery) -- Fetch delivery address
//           ''')
//           .eq('id', int.parse(widget.orderId))
//           .single();

//       // Fetch medicine items with a join
//       final medicineResponse = await _supabase
//           .from('order_medicines')
//           .select('''
//             medicine_id(id, name, price, description)
//           ''')
//           .eq('order_id', int.parse(widget.orderId));

//       setState(() {
//         orderDetails = orderResponse;
//         deliveryDetails = orderResponse['delivery'] as Map<String, dynamic>? ?? {};
//         medicineItems = List<Map<String, dynamic>>.from(medicineResponse.map((item) {
//           final medicine = item['medicine_id'] as Map<String, dynamic>? ?? {};
//           return {
//             'medicine': {
//               'id': medicine['id'],
...
//             },
//             'quantity': 1, // Default quantity; adjust if stored elsewhere
//           };
//         }));
//         isLoading = false;
//       });
//       print('Order Details: $orderDetails');
//       print('Medicine Items: $medicineItems');
...
//                                 Text(
//                                     deliveryDetails?['localisation_delivery'] ??
//                                         'No Delivery Address'),
//                                 const Spacer(),
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  Future<void> _acceptOrder() async {
    try {
      ...
      setState(() {
        if (orderDetails != null) orderDetails!['statut'] = 'completed';
      });
      Navigator.pop(context); // Return to refresh the order list
    } catch (error) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to accept order: $error')),
      );
