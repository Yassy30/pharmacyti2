...
//             user_id:User(full_name, address, role)
//           ''')
...
//           .select('''
//             medicine_id:medicines(name, price, quantity, description)
//           ''')
  ...
  Future<void> _fetchOrderDetails() async {
    ...
    try {
      ...
      final medicineResponse = await _supabase
          .from('order_medicines')
          .select('''
            medicine_id(id, name,quantity, price, description)
          ''')
          .eq('order_id', int.parse(widget.orderId));
                          ...
                          Column(
                            children: medicineItems.map((item) {
                              ...
                              return _buildMedicineItem(
                                medicine['name'] ?? 'Unknown',
                                medicine['description'] ?? 'N/A',
                                '${item['quantity']}x', // Adjust quantity format
                                '₹${medicine['price']?.toString() ?? '0'}',
                                '₹0', // No discount data, set to 0 or fetch if available
                              );
                            }).toList(),
                          ),
                          titleSuffix: Text(
